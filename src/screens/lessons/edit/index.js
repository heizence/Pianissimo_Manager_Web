...
const EditLesson = () => {
  ...
  // 해당 레슨에 등록된 강사 목록 불러오기
  const getEachLessonRequest = async () => {
    try {
      ...
      const res = await API.getEachRegisteredLesson({
        blockId: `${date}&${startTime}`,
      });

      if (res.statusCode === 200) {
        console.log("getEachLessonRequest response : ", res.data);
        setSelectedInstructors(res.data);
      } else {
        alert("요청 중 에러가 발생하였습니다. 잠시 후 다시 시도해 주세요");
      }
    } catch (error) {
      console.error("getEachLessonRequest error : ", error);
    }
  };

  // 레슨 등록하기
  const editLessonRequest = async () => {
    if (!selectedInstructors.length) alert("강사를 추가해 주세요.");
    else {
      try {
        const res = await API.editLesson({
          date: params.date,
        ...
        if (res.statusCode === 200) {
          //console.log("editLessonRequest response : ", res.data);
          alert("수정되었습니다.");
        ...
        console.error("editLessonRequest error : ", error);
      }
    }
  };
