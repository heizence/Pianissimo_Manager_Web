...
const LessonHistoriesModal = ({ studentId, isOpen, setIsOpen }) => {
  ...
  */
  const getLessonHistoriesRequest = async (pageIndex = 0) => {
    try {
      const res = await API.getLessonHistories({ studentId, pageIndex });
        ...
        <div id="eachStudent_modal_table">
          ...
          <div id="eachStudent_modal_tableHeader">
            <span
              ...
              id="eachStudent_modal_eachInstructorName"
            >
              강사
            </span>
            <span
              className="eachStudent_modal_tableEachBlock"
              id="eachStudent_modal_eachLessonDate"
            >
              레슨 날짜
            </span>
            <span
              className="eachStudent_modal_tableEachBlock"
              id="eachStudent_modal_eachLessonTime"
            >
              레슨 시간
            </span>
          </div>
              ...
              return (
                <div key={index} id="eachStudent_modal_tableEachContent">
                  ...
                  <span
                    ...
                    id="eachStudent_modal_eachLessonTime"
                  >
                    {`${each.lessonTime}:00 ~ ${each.lessonTime + 1}:00`}
                  </span>
                </div>
              );
            })
          ) : (
            <div id="eachStudent_modal_noContents">레슨 내역이 없습니다.</div>
          )}
        </div>
      </div>
      {/* 페이지 버튼 */}
...
export default LessonHistoriesModal;
